...
import { getFirestoreUser } from '../lib/connection-manager.js';
...
export default async function handler(req, res) {
  ...
  if (!wss) {
    ...
    wss.on('connection', async (ws, request) => {
      ...
      const userId = url.searchParams.get('userId') || request.headers['x-api-key'];

      if (!userId) {
        ws.send(JSON.stringify({ error: 'Missing userId or x-api-key' }));
        ws.close();
        return;
      }

      try {
        // Verify user exists and get IMAP config
